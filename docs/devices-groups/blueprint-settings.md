...
When you click the Blueprint link, you can view/edit various categories of the settings depending on your selection. To navigate to a different setting category, simply choose a different option from the drop-down.
...
**Multi-app Mode:** Customize apps and settings available on the home screen

**Kiosk Mode:** Dedicate the device to one application for single-purpose functionality
...
:::tip
You cannot set Kiosk Mode when the Android launcher is selected.
:::

### Launcher

You can choose between the Esper launcher or the Android launcher. Most users will use the default setting: The Esper Launcher. This will take administrative control of the device, and only approved applications will be installed or used. All the features of Esper Agents will be functional. 

If you select the Android Launcher, the device user can exit the Esper Agent as they would close any other Android application. The device screen will look and act much like a regular Android device. Esper Agent will continue to run in the background and can be seen in the notification bar. It will provide device management, but many management features will not be available.
...
:::tip
Typically, you need to reboot if switching the device between the launchers.
:::

### Run-time Permissions

Run-time Permissions determine enforcement of the run-time rules across all the applications — Enterprise applications installed by Esper, in-ROM applications, and applications installed via Managed Google Play. This ensures your control of the user experience on the device regarding how permissions are granted.
...
-   Allow Automatically: This rule will grant all the permissions for any application requests without showing a prompt to the user. Allow automatically is the typical setting for single-purpose solutions using a fully trusted, customer-supplied application.
    
-   Ask User: This keeps the default behavior intact regarding permission management. All applications on the device will show a prompt to the user to request permission. For example, before accessing a device module, all applications prompt the user with a message such as “Allow application X to access Gallery? Allow/Deny. Ask user options might not be ideal in some solutions, especially Kiosk-based applications. It can help in certain unusual situations dealing with App permissions.
...
This toggle button allows the installation of applications from unknown sources on the device. The user needs to follow these instructions to enable this on the device: Settings >Apps and Notifications > Special App Permissions> Install Unknown Applications and choose the route from which they wish to install the applications.

For example, if the device user chooses Chrome, they will download and install applications from the Chrome browser on the device. If turned Off, the device user will not be allowed to install applications from unknown sources.

### Allow Application Uninstall

This feature blocks the device user from uninstalling an application. For devices below Andriod 6.0, turning the toggle button off will block app uninstallation. This is especially important for private apps.

  

**Note:** Application Uninstall setting is unnecessary if you run Android 6.0 or above.

  

:::tip

Applications won't uninstall in the Kiosk Mode. We recommend setting the Blueprint to Multi-app Mode and removing the old Kiosk app from the Blueprint.
...
You can upload an APK, choose from an uploaded application and version, add an application from Google Play, or set the state of a pre-installed app. For a pre-installed application, you have the following options:
 
![](./images/blueprintSettings/5-state.png)

Show: will display the application on the home screen

Hide: will hide the application on the home screen.

Disable: will prevent the application from running in the background
...
If you want the device user to use Wi-Fi on the device, set the Wi-Fi toggle button to On; if you don’t, set it to Off.

:::tip
This setting works only with cellular network-active devices. If no cellular network connectivity exists, Esper will turn Wi-Fi on automatically after 10s.
:::

### Allow Bluetooth

If you want the device user to use Bluetooth on the device, set the Bluetooth toggle button to On; if you don’t, set it to Off.

### Allow SMS

SMS controls sending and receiving of SMS or text messages. If SMS is set to Off, the device cannot send or receive text messages from any application. Set SMS to On if you wish to enable sending and receiving text messages.

### Allow NFC

If you want device users to use Near-Field Communication (NFC) on their devices when supported, set NFC to On; if you don’t, set it to Off. If any applications require NFC, make sure this setting is On.
...
All numbers must be preceded by a +country code (example, +1 for the USA). A comma must separate all numbers.
...
2. Fill in the Contact's information according to the header (example, Phone Number, Tag, Tag, Tag)  
    a. Please make sure that the Country Code is included in the Phone Number (example, +1)  
    b. Phone numbers should not include parentheses or hyphens (example, +18881234567)  
    c. Tag should not contain any Numeric or special characters (example, hyphens, quotes, etc.)
...
When you add a Wi-Fi access point to the Blueprint, you need to enter the details of the Wi-Fi access point. To add, click Add Wi-Fi Access Point link.
...
When adding a new access point, you have four fields to configure: Wi-Fi SSID, Wi-Fi Password, Wi-Fi Security type, Hidden. The Wi-Fi SSID and Wi-Fi Password fields are text fields for the SSID and password. Select the security type from the drop-down menu. There are four possible choices:
...
In Android 10 and above, when a Wi-Fi access point is added directly from the device, it is recommended that GPS is ON so that Wi-Fi access point can reflects on the Console.
...
Android Debug Bridge (ADB) is a command-line tool that provides a Unix shell to communicate with an Android device. If the ADB Access option is checked in the Blueprint, you may turn On ADB via Device > Settings for a configurable period. If the ADB option is unchecked in the Blueprint, you cannot turn On ADB from the device settings. Opening the ADB port is required on most stock Android devices before the Esper Agent can securely connect to the Esper Cloud for ADB debugging.
...
Factory Reset controls if users can perform a factory reset using the Android Settings App. When Factory Reset is Off, the user will not be able to complete a factory reset on the device. This does not prevent a hard-key factory reset, see [Factory Reset Protection](./frp.md) to prevent a hard-key factory reset.

Disabling Factory Reset from System Settings will also remove User Factory Reset configurations from the User Mode in Esper Settings App.
...
:::tip
When the Blueprint is applied with password type "None," but the user already sets a password on the device, the device will get priority. You can remove the password from the device remotely using our API. Contact [Esper team](https://support.esper.io/) for more information.
:::
...
1. App running in Kiosk Mode: Only the Kiosk app will be visible along with the back key in the navigation bar. No status bar.
...
2. App running in Mult-app Mode: The status bar and a notification bar will be shown along with a drop-down to show notifications.
...
This setting controls the user's access to settings available via a hidden dock on the device. Esper Setting App has two modes:
...
-   Storage: Helps grant permissions to any directories on the device. On Android 10 or below devices, it is advised that the Console admins inform the users to grant permissions to the root folder. On Android 11 devices, permission cannot be granted to the root folders and download folder.
...
When ON, this setting lets you input a valid JSON code.
...
This section lets you adjust the sound settings of the devices in the group. Use the sliders to adjust the alarm, notification, music, and ring volumes.
...
**Disable Update:** Disables OS updates on the device. Updates will default to the Postpone Installation option if this option is unavailable. This feature may require a supervisor plug-in, Knox-enabled device, or Lenovo device with CSDK support.
